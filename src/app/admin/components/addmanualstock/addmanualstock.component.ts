...
@Component({
  selector: 'app-addmanualstock',
  templateUrl: './addmanualstock.component.html',
  styleUrls: ['./addmanualstock.component.css']
})
export class AddmanualstockComponent implements OnInit {
  ...
  private loadAllStock(){
    this.stockService.getAll().subscribe(stock => { this.stocks = stock.sort((a, b) => a.date > b.date ? -1 : a.date < b.date ? 1 : 0)
      ...
      this.liquidStocks=[];
       ...
       if(ele.iscapital==true && ele.ismanual==true){
           this.capitalStocks.push(ele);
       }
       else if(ele.isliquid==true && ele.ismanual==true){
