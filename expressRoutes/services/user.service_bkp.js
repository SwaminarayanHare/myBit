...
/*db.createView (
    ...
    [
      ...
      { $project: { "subscriptions._id": 0, "subscriptions.userid": 0 } }
    ]
 )
 db.userdetails.find().toArray(function (err, users) {
    if (err) deferred.reject(err.name + ': ' + err.message);
    deferred.resolve(users);
});*/
...
*/
    ...
    MongoClient.connect(connectionString, function(err, db) {
        if (err) throw err;
        var dbo = db.db("mytradnix");

        dbo.createView (
            "userdetails",
            "users",
            [
              { $lookup: { from: "subscriptions", localField: "_id", foreignField: "userid", as: "inventory_docs" } },
              { $project: { "inventory_docs._id": 0, "inventory_docs.userid": 0 } }
            ]
         )
        ...
        dbo.userdetails.find().toArray(function(err, res) {
          if (err) throw err;
          deferred.resolve(JSON.stringify(res));
          db.close();
        });
